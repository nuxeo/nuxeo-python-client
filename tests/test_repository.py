...
from nuxeo.exceptions import BadQuery, HTTPError, \
    UnavailableConvertor, NotRegisteredConvertor, \
...
def test_bogus_converter(monkeypatch, server):
    ...
    with Doc(server) as doc:
        ...
        assert str(e.value).startswith("ConvertorNotRegistered: conversion with options")
...
def test_unavailable_bogus_converter(monkeypatch, server):
    ...
    def get(*args, **kwargs):
        ""Mimic the error message when a converter is not available.""
        raise HTTPError(message=f"{converter} is not registered or unavailable")
    ...
    with Doc(server) as doc:
        ...
        assert  "is not registered or unavailable" in str(e.value)
"""
